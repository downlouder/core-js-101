...
function isCreditCardNumber(ccn) {
  const cardNumberSum = ccn.toString().split('').reverse().map((value) => Number(value))
    .reduce((acc, value, index) => {
      let digit = value;
      if (index % 2) {
        digit *= 2;
      }
      return acc + Math.floor(digit / 10) + (digit % 10);
    }, 0);
  return (cardNumberSum * 3) % 10 === 0;
}
...
function getDigitalRoot(num) {
  while (num > 9) {
    return getDigitalRoot(num.toString().split('').map((value) => Number(value)).reduce((acc, curVal) => acc + curVal));
  }
  return num;
}
 ...
 * https://en.wikipedia.org/wiki/Radibracketx
 *
 * @param {number} num
...
function getCommonDirectoryPath(pathes) {
  ...
  while (pathesArray.length !== 0) {
    const temp = pathesArray[0].shift();
    for (let i = 1; i < pathesArray.length; i += 1) {
      if (pathesArray[i].shift() !== temp) {
        return result;
      }
    }
    result += temp;
...
function getMatrixProduct(m1, m2) {
  ...
  const resultArray = new Array(row);
  for (let i = 0; i < row; i += 1) {
    resultArray[i] = new Array(col);
  }
  for (let i = 0; i < row; i += 1) {
